...
class TestPaymentsTransfer:
    ...
    @pytest.mark.parametrize("amount, expected_status, expected_error_type",
        [
            ...
            (-10, 422, "greater_than"),                     # valor negativo inválido (fuera del dominio)
            (-0.01, 422, "greater_than"),                   # apenas por debajo del límite inferior
            (1_000_000, 202, None),                         # valor alto válido (prueba de límite superior / stress)
            ...
            ("100", 202, None),                             # coerción de string numérico, podría ser interpretado como 100 o lanzar un error de validación
            (True, 202, None),                              # coerción a 1, podría ser interpretado como 1 o lanzar un error de validación
        ]
    )
